...
public class KThread {
	...
	private static class PingTest implements Runnable {
		...
		public void run() {
			for (int i = 0; i < 5000; i++) { // You may adjust the number of iterations
	...
	public static void selfTest() {
		...
		/**
		 * Allocate a new thread setting the target to point to a PingTest object whose
		 * run method will be called when the newly created thread executes.
		 */
		...
		/**
		 * Execute fork() to begin execution of the new thread (putting it in the ready
		 * queue). This new thread will eventually execute the PingTest object's run()
		 * method when scheduled. The current thread (that created this new thread) will
		 * return from the fork() method call resuming its own execution, thus, giving
		 * us 2 concurrent thread.
		 */
		th1.fork();
		/**
		 * The current thread that returned from fork() (creating the new thread th1
		 * above) will then create its own PingTest object and execute the run method.
		 * So now we have two threads running the PingTest's run() method.
		 */
		new PingTest(0).run();
		/**
		 * Current thread calls join() on the newly created thread thus going to sleep
		 * till th1 finishes.
		 */
		th1.join();
		/**
		 * Try to join with th1 again. This should immediately return as th1 should have
		 * already finished.
		 */
		// joins the same thread twice. nachos asserts.
		// th1.join();
	}
